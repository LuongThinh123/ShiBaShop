const HandleEvent = {
  User: {
    ...
    Save: function (item) {
      ...
      if (!Validate.isLength(user.username, options.user.username)) {
        temp.push({ type: 'warning', icon: icon['warning'], message: lang.usernameNotValid.text });
        row.querySelector('.username').style.backgroundColor = 'var(--red)';
      }
      if (user.address == '' || user.fullname == '' || user.phone == '') {
        temp.push({ type: 'warning', icon: icon['warning'], message: lang.inputEmpty.text });
        user.address == ''
          ? (row.querySelector('.address').style.backgroundColor = 'var(--red)')
          : null;
        user.fullname == ''
          ? (row.querySelector('.fullname').style.backgroundColor = 'var(--red)')
          : null;
        user.phone == ''
          ? (row.querySelector('.phone').style.backgroundColor = 'var(--red)')
          : null;
      }
      if (!temp.length == 0) {
        temp.forEach((error) => {
          toast(error.type, error.icon, error.message);
        });
        console.log(temp);
      } else {
  ...
  Product: {
    ...
    Save: function (item) {
      ...
      if (
        Validate.isNumber(product.price) &&
        Validate.isNumber(product.sale) &&
        Validate.isLength(product.name, options.product.name)
      ) {
        if (confirm('Bạn có thật sự muốn lưu không ?')) {
          let result = ProductModel.updateProduct(id, product);
          toast(result.message.type, icon[result.message.type], result.message.text);
          if (result.message.type == 'warning' || result.message.type == 'danger') {
            row.querySelector('.nameProduct').style.backgroundColor = 'var(--red)';
          }
          if (result.message.type == 'success') {
        ...
        if (!Validate.isNumber(product.price) || !Validate.isNumber(product.sale)) {
          toast('warning', warning, lang.priceNotValid.text);
        }
        !Validate.isNumber(product.price)
          ? (row.querySelector('.priceProduct').style.backgroundColor = 'var(--red)')
          : null;

        !Validate.isNumber(product.sale)
          ? (row.querySelector('.priceSaleProduct').style.backgroundColor = 'var(--red)')
          : null;
        if (!Validate.isLength(product.name, options.product.name)) {
          row.querySelector('.nameProduct').style.backgroundColor = 'var(--red)';
          toast('warning', warning, lang.nameProductNotValid.text);
        }
      }
    },
    Remove: function (item) {
    ...
    InnerCategoryInFilter: function () {
      let categories = CategoryModel.getAll();
      let listButton = '<button class="btn-select-category active">All</button>';
      categories.forEach((category) => {
  ...
  Category: {
    ...
    Remove: function (item) {
      if (confirm('Bạn có thật sự muốn xoá không ?')) {
        ...
        PaginationController.Update('category');
        if (length == 0) $('.tmanager-category .previous').click();
        HandleEvent.Product.InnerCategoryInFilter();
      }
    },
