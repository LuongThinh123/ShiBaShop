const ProductModel = {
  ...
  updateProduct: function (id, product) {
      ...
      for (let document of collection) {
        if (document.id == id) {
          ...
          if (
            ...
            product.price == Number(document.price) &&
            product.sale == Number(document.sale) &&
    ...
    } else return { status: false, message: lang.nameProductNotValid };
  },
    ...
    ProductModel.getAll().forEach((product) => {
      let condition =
        rate < 0
          ? from <= product.sale &&
            product.sale <= to &&
            product.category.toLowerCase().match(patternCategory) !== null &&
            product.name.toLowerCase().match(patternName) !== null
          : from <= product.sale &&
            product.sale <= to &&
            product.category.toLowerCase().match(patternCategory) !== null &&
            product.name.toLowerCase().match(patternName) !== null &&
            product.rate == rate;
      if (condition) products.push(product);
    });
    return products;
  },
