...
window.onhashchange = function () {
  ...
  switch (window.location.hash.split('-')[0]) {
      ...
      if (prevHash[0].split('-')[0] != '#shop') {
        ...
        if (hash[1]) {
          if (hash[1].split(':')[0] == 'search') {
            ...
            $('#max-input').value = 99999;
            $$('.theloai input').forEach((input) => {
              if (input.value == categoryName.toUpperCase()) input.checked = true;
            });
            $('#filtertitle').click();
          }
        }
    ...
    case '#about':
      ...
      if ($('nav a.active')) $('nav a.active').classList.remove('active');
      renderAbout.start();
      $('nav a.about').classList.add('active');
      break;
...
const renderDetail = {
  start(product) {
    ...
    $('.nav__item--disable').innerHTML = product.name;
    InitEventDetail();
